...
function changeBackColor(event) {
  event.target.style = "background-color:#efefef;color:black";
}

sayHelloButton.addEventListener("mouseout", changeBackColor);
// PROBLEM 4
/*
...
const repeatMyParam = () => {
  //YOUR CODE HERE
  axios.get("http://localhost:3000/repeat/hola").then((res) => {
...
function queryTest() {
  axios
    .get("http://localhost:3000/query-test/?message=hello&notamessage=goodbye")
...
function createFood(event) {
  ...
  axios.post("http://localhost:3000/food", body).then((res) => {
    ...
    for (let i = 0; i < res.data.length; i++) {
      let newElement = document.createElement("p");
      newElement.textContent = res.data[i];
      documentBody.appendChild(newElement);
    }
  });
}
